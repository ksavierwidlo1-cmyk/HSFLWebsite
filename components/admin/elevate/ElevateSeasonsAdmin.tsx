...
export default function ElevateSeasonsAdmin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId ? `/api/elevate/seasons/${editingId}` : '/api/elevate/seasons';
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/elevate/seasons/${id}`, { method: 'DELETE' });
      if (res.ok) {
        await fetchSeasons();
      } else {
        const error = await res.json();
